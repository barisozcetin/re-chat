	...
	render() {
		...
		const loginWithEmail = (
			<form onSubmit={this.onLogin}>
				...
				<div className="field">
					<p className="control">
						<input type="submit" className="button is-success full-width" value="Login" />
					</p>
				</div>
			</form>
		);
		...
		return (
			<div className="login__container animated fadeIn p-3 box">
				...
				<p className="m-a">
					Not a member?{' '}
					{this.props.isModal ? <a onClick={this.props.switchComponent}>Sign Up</a> : <Link to="/signup">Sign Up</Link>}
				</p>
			</div>
		);
	}
