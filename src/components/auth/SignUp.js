	...
	render() {
		...
		const loginWithEmail = (
			<form onSubmit={this.onSignUp}>
				...
				<div className="field">
					<p className="control">
						<input type="submit" className="button is-success full-width" value="Sign Up" />
					</p>
				</div>
			</form>
		);
		...
		return (
			<div className="login__container animated fadeIn p-3 box">
				...
				<p className="m-a">
					...
					{this.props.isModal ? <a onClick={this.props.switchComponent}>Login</a> : <Link to="/signup">Login</Link>}
				</p>
			</div>
		);
	}
